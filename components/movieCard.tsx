...
function MovieCard({movie}:{movie:Movie}) {
  return (
    <div className="flex-shrink-0 relative cursor-pointer transform hover:scale-105 transition duration-200 ease-out hover:drop-shadow-lg">
      ...
      {/* we are creating a funtion because we wil ge the path as
      /film/123/poster.jpeg they will not give u the begining of the path so we
      need to create a function to get the entire path   ----- talking about getImagePath funtion*/}

      <Image
        src={getImagePath(movie.backdrop_path || movie.poster_path)} //we wil check the backdrop path if that is not present it finds poster-path
        alt={movie.title}
        width={1920}
        height={1080}
        key={movie.id}
